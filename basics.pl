/** ---------------------------------------------------------
  This file contains common code for the three search algorithm
  implemented in:
    astar.pl,
    astarCC.pl, and
    idastar.pl.
...
callEqFn(StateEqFn, S1, S2) :-
	%% create a call to StateEqFn on S1 and S2 return true if they're equal.
	E =.. [StateEqFn, S1, S2], !, E.
...
/* predicates defined for SICStus */
:- use_module(library(lists)).


writeln([]) :- nl.
writeln([X | Rest]) :- write(X), writeln(Rest).
